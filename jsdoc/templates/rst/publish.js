...
const rstHelper = require('./rstHelper');
...
function updateItemName(item) {
    ...
    if (attributes && attributes.length) {
        itemName = util.format( '%s `%s`::attributes', itemName,
            attributes.join(', ') );
    }
...
function buildAttribsString(attribs) {
    ...
    if (attribs && attribs.length) {
        attribsString = rstsafe( util.format('`(%s)`:attribs ', attribs.join(', ')) );
    }
...
function addSignatureReturns(f) {
    ...
    f.signature = `\`${f.signature.replace(/([()])/g, '\\$1') || ' '}\`:signature: \`${returnTypesString || '\\ '}\`:type-signature:`;
}

function addSignatureTypes(f) {
    const types = f.type ? buildItemTypeStrings(f) : [];

    f.signature = `${f.signature || ' '} \`${types.length ? ` :${types.join('|')}` : ' '}\`:type-signature:`;
}
...
function addAttribs(f) {
    ...
    f.attribs = util.format('`%s`:type-signature:', attribsString || '\\ ');
}
...
function generateSourceFiles(sourceFiles, encoding = 'utf8') {
    Object.keys(sourceFiles).forEach(file => {
        ...
        sourceOutfile.replace(/\.html$/i, '.rst');
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    view.rstsafe = (x) => x;
    view.makeTable = rstHelper.makeTable;
